...
- 2026-05-27: Visit-log generation now uses `gpt-5.4-mini` for the runtime visit-log model. Planner candidates are filtered by department tags before product/detail selection, with respiratory candidates limited to pneumonia/respiratory infection/tuberculosis/intake difficulty/recovery nutrition/outpatient anemia and OB candidates limited to postpartum or OB-surgery anemia/recovery contexts. Validator adds `DEPARTMENT_MISMATCH`, blocking respiratory outputs that mention delivery/postpartum/pregnancy and OB outputs that mention ICU/critical/neurosurgery/respiratory infection/pneumonia/tuberculosis unless the user wrote that special case in the raw memo. Repair now replans for department mismatch. `scripts/test-pipeline.mjs`, `pnpm run typecheck`, and `pnpm run build` passed.
- 2026-05-27: Department realism validation was refactored away from pairwise hardcoded forbidden phrases. `clinical-domain.ts` now defines reusable clinical domains, department allowed-domain rules, domain term detection, and candidate/domain fit checks. Planner candidates carry `clinicalDomains`, validator calls `findMismatchedClinicalDomain`, and tests assert respiratory/OB mismatches through the generic taxonomy, including raw-memo special-case exemptions. `scripts/test-pipeline.mjs`, `pnpm run typecheck`, and `pnpm run build` passed.
- 2026-05-27: Visit-log generation hardcoded department theme fallbacks were consolidated into `clinical-domain.ts`. The legacy `DEPT_FEATURE_RULES`/`disallowedThemes` table and planner `blockedDepartmentTags` were removed, AI final correction now uses `buildClinicalDomainConstraint` and `removeMismatchedClinicalDomainSentences`, and fallback visit candidates carry `clinicalDomains` with `candidateFitsDepartment` filtering. `scripts/gsd-context-hygiene.mjs` is still absent in this clone, so it could not be run. `scripts/test-pipeline.mjs`, `pnpm run typecheck`, and `pnpm run build` passed.
