...
① [이전 연결 - 선택] 지난 방문과 연결할 내용이 있을 때만 사용. 없으면 생략.
   - ✅ 올바른 표현: "지난번에 페린젝트 급여조건 디테일 드렸는데 처방해봤는지 확인하고,"
   - ✅ 올바른 표현: "지난번에 위너프 데이터 드렸는데 반응 어떠신지 여쭤보고,"
   - ❌ 금지: "~흐름이어서", "~흐름으로", "다시 봄", "다시 말씀드림", "다시 디테일"
② [오늘 핵심] 어떤 제품의 어떤 특장점을 전달했는지 구체적으로. 위에 제공된 제품 특장점이 있으면 그 화법을 녹여낼 것 (원문 복붙 금지).
③ [오브젝션 핸들링 - 약 30% 확률로 자연스럽게 포함] 실제 영업 현장처럼 의사가 질문이나 반대 의견을 낼 수 있음. 교수 반박 패턴이 있으면 그걸 활용하고, 없으면 해당 과/제품에 현실적으로 나올 법한 반박/질문을 만들어서 → "~라고 하심. ~라고 말씀드림. 어느 정도 공감하심" 형태로 작성. 나머지 70%는 생략.
④ [교수 반응] 교수가 어떻게 반응했는지 한 줄로.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

주의:
- 입력 메모에 없는 내용 추가 금지
- 교수 성향/처방 경향은 텍스트로 서술 금지. 어조에만 반영.
- 빈 줄, 제목, 구분선 없이 이어서 작성

★ 절대 규칙:
1. 반드시 230자(한글 기준) 이내. 230자 넘으면 전체 다시 쓸 것.
2. 큰따옴표("), 작은따옴표(') 절대 사용 금지.

응답 형식:
===영업일지===
(230자 이내. 위 흐름대로. 설명문 금지)

===다음방문전략===
(120자 이내. 반드시 "다음방문시에는", "다음번에는", "다음에는" 중 하나로 시작. 구체적 제품명 + 해당 진료과/환자군과 연결해서 작성. 라벨/구분자 다시 쓰지 말 것)

제목, 구분선, 라벨 절대 붙이지 마세요.`;
...
① [이전 연결 - 선택] 지난 방문과 연결할 내용이 있을 때만 사용. 없으면 생략.
   ...
   - ✅ 올바른 표현: "지난번에 위너프 데이터 드렸는데 반응 어떠신지 여쭤보고,"
   - ❌ 금지: "~흐름이어서", "~흐름으로", "다시 봄", "다시 말씀드림", "다시 디테일"
② [오늘 핵심] 해당 과/환자군에 적합한 제품의 구체적 특장점 전달. 위에 제공된 제품 특장점 화법을 자연스럽게 녹여낼 것 (원문 복붙 금지).
③ [오브젝션 핸들링 - 약 30% 확률로 자연스럽게 포함] 실제 영업 현장처럼 의사가 질문이나 반대 의견을 낼 수 있음. 교수 반박 패턴이 있으면 그걸 활용하고, 없으면 해당 과/제품에 현실적으로 나올 법한 반박/질문을 만들어서 → "~라고 하심. ~라고 말씀드림. 어느 정도 공감하심" 형태로 작성. 나머지 70%는 생략.
④ [교수 반응] 교수가 어떻게 반응했는지 한 줄로.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- 전 방문 전략이 있으면 그것을 이번 방문에서 실행한 것으로 구성.
- 병원/과 특성에 맞는 현실적 내용으로.
- 빈 줄, 제목, 구분선 없이 이어서 작성.
- 결과는 현장에서 적은 짧은 메모. 보고서 절대 금지.
- 페린젝트는 반드시 1회 투여로 표기.

★ 절대 규칙:
1. 반드시 230자 이내. 230자 넘으면 전체 다시 쓸 것.
2. 큰따옴표("), 작은따옴표(') 절대 사용 금지.
...
===영업일지===
(230자 이내. 위 흐름대로. 설명문/보고서 금지. 큰따옴표 작은따옴표 사용 금지)

===다음방문전략===
(120자 이내. 반드시 "다음방문시에는", "다음번에는", "다음에는" 중 하나로 시작. 구체적 제품명 + 해당 진료과/환자군과 연결. 라벨/구분자 다시 쓰지 말 것)`;
