...
① [이전 연결 - 선택] 지난 방문과 연결할 내용이 있을 때만.
   예: "지난번에 페린젝트 급여조건 디테일 드렸는데 처방해봤는지 여쭤보니 ~하셨다고 하심. 이어서~"
   예: "지난번에 위너프 데이터 드렸는데 반응 어떠신지 여쭤보고,"
   ★ 이전 연결을 꺼냈으면 그 주제를 완결(확인/반응 포함)한 뒤 다음 주제로 넘어갈 것. 주제 중간에 갑자기 다른 얘기로 넘어가지 말 것.
   없으면 생략.
② [오늘 핵심] 어떤 제품의 어떤 특장점을 전달했는지 구체적으로. 제공된 제품 특장점 화법 자연스럽게 녹여낼 것 (원문 복붙 금지).
   ★ 오늘 주제가 여러 개면 "이어서", "추가로" 등으로 자연스럽게 연결. 각 주제는 짧게 완결 후 다음으로.
③ [오브젝션 핸들링 - 30% 확률] 의사가 질문/반대 의견을 낼 수 있음. 교수 반박 패턴 있으면 활용, 없으면 해당 과/제품에 현실적으로 나올 법한 반박을 만들어서 포함. 형태: "~라고 하심. ~라고 말씀드림. 어느 정도 공감하심". 70%는 생략.
④ [교수 반응] 교수 반응 한 줄로.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
}
...
3. 원본에 제품 디테일이 없으면 → 이 과(${doctor.department})에 맞는 제품 디테일 포인트 1개만 자연스럽게 추가.
4. 오브젝션 핸들링 → 30% 확률로 추가 (아래 흐름 규칙 참고).
5. 다음방문전략 → 반드시 작성 (필수).
