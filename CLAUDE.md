...
### 🎯 토큰 절약 가이드
**효율성을 해치지 않으면서 토큰 소모를 최소화:**

#### 파일 읽기
- `limit`/`offset` 사용: 큰 파일은 필요한 라인 범위만 읽기
  ```bash
  Read(file_path: "ai.ts", limit: 50, offset: 0)  # 처음 50줄
  ```
- **1회 읽기 원칙**: 같은 파일을 여러 번 읽지 말고, 필요한 모든 정보를 한 번에 읽기
- 이미 읽은 파일의 내용은 요청 중 재사용

#### 도구 조합
- **Grep → Read**: 먼저 Grep으로 위치 찾은 후 정확한 라인 범위만 Read
- **Glob → Read**: 파일 경로 확인 후 필요한 것만 열기
- Agent는 신중하게: 독립적 context 필요하므로 정말 필요할 때만 사용

#### 코드 작업
- **한 번에 여러 파일 수정**: 분리된 커밋보다 연관된 변경을 한 commit으로
- **Plan 모드**: 복잡한 작업 전 한 번에 계획 수립 (불필요한 재작업 방지)
- **Git 명령 최소화**: 상태 확인은 한 번의 git status + git diff로 처리

#### 응답
- 명확한 변경은 간단히: 코드 리뷰 불필요하면 생략
- 사용자 질문에만 정확하게 답변
- 자명한 상황 설명 축약

#### 캐싱 전략
- 파일 내용 변경 후 다시 Read하지 말기 (Edit/Write 성공 = 변경 확정)
- 이전 응답에서 언급한 정보 재사용
